...
networkName <- c("Lazega-blocking","AUCS", "MoscowAthletics2013","EU")
mainDirectory <- beta[1]
scritpType<-"SIS&SIR_blocking"
countryDirectory <-"Mnoznikx1"
...
sdAllS*100
plot(1:size, allS[1:size],xlab= "time", ylab="% actors",xlim=c(0,size), ylim=c(0,100),pch=19, col="blue")
points(1:size, allR,pch=19,col="green")
points(1:size, allI, col= "red")
axis(side=2, at=seq(0,100,5))
axis(side=1, at=seq(0,size,5))
legend(130, 90, legend=c( "S","I", "R"),col=c("blue", "red", "green"), pch=19)

arrows(1:size,allI[1:size]+sdAllI[1:size],1:size,allI[1:size]-sdAllI[1:size],  angle=90, code=3, length=0.1, col="grey")
